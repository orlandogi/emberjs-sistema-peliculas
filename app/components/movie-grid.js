...
export default class MovieGridComponent extends Component {
    ...
    if (this.filterState2 !== 'todos') {
      movies = movies.filter((movie) => {
        return movie.generos
          .map((g) => g.toLowerCase())
          .includes(this.filterState2.toLowerCase());
      });
    }
    if (this.filterType2 !== 'todos') {
      movies = movies.filter(
        (movie) =>
          movie.strEstado.toLowerCase() === this.filterType2.toLowerCase(),
      );
    }
  ...
  @action async deleteMovie(movie) {
    try {
      ...
      axios.delete(
        `https://backend-express-production-be7d.up.railway.app/api/pelicula/${movie}`,
      ));
          ...
          if (etiquetas.length >= 1) {
            if (!valorimagen.src.includes('images/pelicula-icon.jpg')) {
              try {
                ...
                formData.append(
                  'intDuracion',
                  document.getElementById('numberDuration2').value,
                );
                if (document.getElementById('opDisponible2').selected) {
                  formData.append(
                    'idEstadoPelicula',
                    document.getElementById('opDisponible2').value,
                  );
                } else {
                  formData.append(
                    'idEstadoPelicula',
                    document.getElementById('opRetirada2').value,
                  );
                }
                ...
                if (
                  fileInput.files &&
                  fileInput.files[0] &&
                  this.imagenSource != valorimagen.src
                ) {
                ...
                const response = await ((document.getElementById(
                  'enviarMovie2',
                ).disabled = true),
                (document.getElementById('cerrarMovieG').disabled = true),
                (document.getElementById('cerrarGMovie').disabled = true),
                axios.put(
                  `https://backend-express-production-be7d.up.railway.app/api/pelicula/${this.usuId2}`,
                  formData,
                  {
                    headers: {
